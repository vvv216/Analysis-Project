...
public class EnergyUse extends AbstractAnlyzer{
    ...
    public void analysis() {
        data = "Energy use (kg of oil equivalent per capita):\n";
        JsonArray array = getResponse(ENERGY_USE_STR_0);
    ...
    private void createLineChart(JsonArray jsonArray){
        ...
        for (int i = 0 ; i < array.size(); i++){
            ...
            mDataset.addValue(value,"Energy use(kg of oil equivalent per capita)",year+"");
            //join value to data
            data+= year+": Energy use is "+value+"\n";
        }
        //create line chart
    ...
    private void createBarChart(JsonArray jsonArray){
        ...
        for (int i = 0 ; i < array.size(); i++){
            ...
            mDataset.setValue(value,"Energy use (kg of oil equivalent per capita)",year+"");
        }


//        create bar chart
        JFreeChart chart = ChartFactory.createBarChart("Energy use",
                "year",
                "kg of oil equivalent per capita",
                mDataset,
                PlotOrientation.VERTICAL,
    ...
    private void createScattChart(JsonArray emArray){
    	...
    	XYSeries series1 = new XYSeries("Energy use");
    	for (int i = 0 ; i < array.size(); i++){
    		//get date
    		int year = array.get(i).getAsJsonObject().get("date").getAsInt();
    		//get data
    	...
    	//create line chart
    	JFreeChart chart = ChartFactory.createScatterPlot("Energy use", "year", "kg of oil equivalent per capita", mDataset1);
    	//get plot
